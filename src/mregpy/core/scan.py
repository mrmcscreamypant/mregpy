...
class DirToScan:
    def __init__(self,path:pathlib.Path,scanner:Scanner):
    ...
    def __load_ignore(self):
        ...
        if ignore.exists():
            ...
            self.ignore = IgnoreFile(ignore)
    
    def __go(self):
        for path in self.path.iterdir():
            if path.is_dir() and not self.ignore.match(path):
                DirToScan(path,self.scanner)
